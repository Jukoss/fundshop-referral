$(document).ready(function() {
	...
	$('.slick_nav').slick({
		...
		arrows: true,
	  	...
	  	asNavFor: '.slick_for',
	  	fade: true,
	  	appendDots: $('.section_epgb_cont_tabnav_dt'),
	  	prevArrow: $('.section_epgb_cont_tabitm_prew'),
    	nextArrow: $('.section_epgb_cont_tabitm_next'),
    	customPaging: function(slider, i) {
	      	return '<button type="button">Step '+(i+1)+'</button>';
	    }
	});*/
	...
	/*$('form select').on("keyup, change", function() {
        let id = $(this).attr('id');
        if (!$("label[for='"+id+"']").css('opacity') == 0) {
            $("label[for='"+id+"']").addClass('visible_label');
        }else if ($(this).find("label").css('opacity') > 0) {
            $("label[for='"+id+"']").removeClass('visible_label');
        }
        console.log(id);
    });
...
$("#calc_fotm_home, #sld_egd_home, .sect_btn_lscr").on('click', function(e){
    $('html, body').animate({
